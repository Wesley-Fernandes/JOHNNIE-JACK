...
export default function Acquisition({data, quantity, complements, setQuantity}:Props) {
  const [price, setPrice] = useState<string>("")
    // biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
    useEffect(()=>{
      const new_price = calculatePrice({price: data.price, quantity, complements})
      setPrice(new_price)
  ...
  return (
    ...
    <CardFooter className='w-full'>
      <Button variant="destructive" className="uppercase font-bold flex-1 border-1 shadow-md bg-gradient-to-t from-green-600 to to-green-400 w-full">Adicionar</Button>
    </CardFooter>
  </Card>
  )
}
