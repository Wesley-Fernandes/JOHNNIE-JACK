...
export async function GET(request:NextApiRequest){
    try {
        ...
        const category:CategoryTypes = url.searchParams.get("category") as CategoryTypes;
        if(!category) return NextResponse.json({message: "Missing category query"}, {status: 400});
